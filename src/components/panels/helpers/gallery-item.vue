<template>
    <div
        class="flex flex-col relative w-full h-full max-h-full justify-center"
        :class="{ 'storylines-modal-active': props.isActive }"
        :style="{ cursor: props.isActive ? 'zoom-out' : 'zoom-in' }"
    ...
    >
        ...
        <div
            v-show="props.alt && props.isActive"
            class="leading-1 text-center mt-2 cursor-text select-text"
            v-html="mdContent"
            @click="preventDefault"
        ></div>

        <!-- The little '?' icon, only shows up if the `credit` prop is provided. -->
        <svg
            class="top-2 left-2 absolute rounded-full bg-white border-2 border-black p-0.5 cursor-pointer"
            ...
            xml:space="preserve"
            v-tippy="{
                content: props.credit,
                placement: 'right',
                toggle: 'mouseenter focus click'
            }"
            tabindex="0"
            v-if="props.credit && !props.isActive"
            @click="preventDefault"
