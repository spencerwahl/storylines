import{fg as H,ha as j,fk as F,fl as O,fv as _,fm as R,gl as v,fx as X,fi as U,fw as V,fy as D,hH as q,fn as B,fo as n,fp as z,fq as k,gW as J,fF as c,ft as f,fA as d,fr as u}from"./story-YnVQjsbo.js";import{u as G}from"./marked.esm-DgqJBp_S-CCvGNfAT.js";import"./main-BqdsHUTV.js";const K=`<?xml version="1.0" encoding="UTF-8"?>
...
`,Z={key:0},ee={class:"flex justify-center"},te={key:0,class:"flex flex-col justify-center text-center"},ae=["innerHTML"],le=["innerHTML"],se={key:3,class:"flex flex-col justify-center text-center"},oe={class:"text-xl mt-20"},re={key:1,class:"p-5"},me=H({__name:"screen",props:{panel:{type:Object,required:!0},payload:{type:Object,required:!0}},setup(i){const s=j(),{t:o}=F(),x=O("iApi"),I=_(),l=i,w=R(()=>s.status),y=R(()=>s.response),P=_(!1),g=v({}),C=v([]),S=v([]);X(()=>{s.status="loading",T(),C.push(x.event.on(U.LAYER_REMOVE,e=>{l.payload.layer?.uid===e.uid&&l.panel.close()})),S.push(V(()=>l.payload.layer.uid,(e,t)=>{e!==t&&(s.status="loading",T())}))}),D(()=>{C.forEach(e=>x.event.off(e)),S.forEach(e=>e())});const T=()=>{P.value=l.payload.layer!==void 0&&!l.payload.layer.isRemoved,l.payload.type==="xml"?N(l.payload.url,[]).then(e=>{if(s.status="success",e!==null){const t=document.createElement("div");t.appendChild(p(`${e.firstElementChild.outerHTML}`)),(l.payload.catalogueUrl||l.payload.url)&&t.appendChild(p(`<h5 class="text-xl font-bold mb-3">${o("metadata.xslt.metadata")}</h5>`)),l.payload.catalogueUrl&&t.appendChild(p(`<p><a style="color: blue;" href="${l.payload.catalogueUrl}" target="_blank">${o("metadata.xslt.cataloguePage")}</a></p>`)),t.appendChild(p(`<p><a style="color: blue;" href="${l.payload.url}" target="_blank">${o("metadata.xslt.metadataPage")}</a> (xml)</p>`)),s.response=t.outerHTML}}):l.payload.type==="html"?b(l.payload.url).then(e=>{s.status=e.status,s.response=e.response}):l.payload.type==="md"&&b(l.payload.url).then(e=>{s.status=e.status,s.response=G(e.response)})},N=(e,t)=>{let a;return l.payload.xmlType&&l.payload.xmlType==="DCAT"?a=x.language==="en"?Y:W:a=x.language==="en"?K:Q,a=a.replace(/\{\{([\w.]+)\}\}/g,(r,m)=>o(m)),g[e]?Promise.resolve($(g[e],a,t)):b(e).then(r=>(g[e]=r.response,$(g[e],a,t)))},$=(e,t,a)=>{let r=null;if(window.XSLTProcessor){const m=new window.XSLTProcessor,L=new DOMParser,M=L.parseFromString(e,"text/xml"),E=L.parseFromString(t,"text/xml");m.importStylesheet(E),a&&a.forEach(h=>m.setParameter("",h.key,h.value||"")),r=m.transformToFragment(M,document);const A={className:"underline text-blue-700 break-all",target:"_blank",validate:{url:h=>/^https?:\/\//.test(h)}};r.firstChild.innerHTML=q(r.firstChild.innerHTML,A)}return r},b=e=>new Promise(t=>{const a=new XMLHttpRequest;a.open("GET",e,!0),a.responseType="text",a.onload=()=>{a.status===200?t({status:"success",response:a.response}):t({status:"error",response:"Could not load results from remote service."})},a.onerror=()=>{t({status:"error",response:"Could not load results from remote service."})},a.send()});function p(e){const t=document.createElement("div");return t.innerHTML=e,t}return(e,t)=>{const a=B("panel-screen");return n(),z(a,{panel:i.panel,ref_key:"el",ref:I},{header:k(()=>[J(c(f(o)("metadata.title"))+": "+c(i.payload.layerName),1)]),content:k(()=>[P.value?(n(),d("div",Z,[u("div",ee,[w.value=="loading"?(n(),d("div",te,c(f(o)("metadata.loading")),1)):i.payload.type==="xml"&&w.value=="success"?(n(),d("div",{key:1,innerHTML:y.value,class:"flex flex-col justify-center max-w-full xml-content"},null,8,ae)):(i.payload.type==="html"||i.payload.type==="md")&&w.value=="success"?(n(),d("div",{key:2,innerHTML:y.value,class:"flex flex-col justify-center max-w-full metadata-view"},null,8,le)):(n(),d("div",se,[t[0]||(t[0]=u("img",{src:"https://i.imgur.com/fA5EqV6.png"},null,-1)),u("span",oe,c(f(o)("metadata.error")),1)]))])])):(n(),d("div",re,[u("span",null,c(f(o)("metadata.label.no.layer")),1)]))]),_:1},8,["panel"])}}});export{me as default};
